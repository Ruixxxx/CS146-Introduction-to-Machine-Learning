...
"Rui Xu 005230642"
"second try with polynomial regression"
...
class PolynomialRegression:
    ...
    def fit(self, X, y, l2regularize=None):
        ...
        self.coef_ = np.dot(np.dot(np.linalg.pinv(np.dot(X.T, X)), X.T), y.T).T
    ...
    def cost(self, X, y):
        ...
        # part d: compute J(w)
        X = self.generate_polynomial_features(X)  # map features
        cost = np.sum(np.square(np.dot(self.coef_, X.T) - y))
...
def main():
    ...
    ## step size = 0.01
    print("--------GD: step size = 0.01--------")
    model1 = PolynomialRegression(m = 1)
    model1.fit_GD(X, y, eta=0.01)
    for i in model1.coef_:
        print("cofficient: %e" %i)
    print("cost: %e" %(model1.cost(X, y)))
    
    ## investigating GD with different step size
    for j in [1e-4, 1e-3, 1e-2, 0.0407]:
        print("--------GD: step size = %f--------" %j)
        start = t.perf_counter()
        model_test = PolynomialRegression(m = 1)
        model_test.fit_GD(X, y, eta=j)
        for i in model_test.coef_:
            print("cofficient: %e" %i)
        print("cost: %e" %(model_test.cost(X, y)))
        end = t.perf_counter()
    ...
    ## GD with step size 1/(1+t)
    print("--------GD: step size = 1/(k+1)--------")
    start = t.perf_counter()
    model_test = PolynomialRegression(m = 1)
    model_test.fit_GD(X, y)
    for i in model_test.coef_:
        print("cofficient: %e" %i)
    print("cost: %e" %(model_test.cost(X, y)))
    end = t.perf_counter()
    ...
    complexity = range(11)
    for c in complexity:
        model = PolynomialRegression(m = c)
        model.fit(X, y)
        RMSE_train.append(model.rms_error(X, y))
        RMSE_test.append(model.rms_error(X_test, y_test))
    plt.figure()
    plt.plot(complexity, RMSE_train, color = "r", label = "training error")
    plt.plot(complexity, RMSE_test, color = "b", label = "test error")
